    ...
    <div class="container-fluid py-4" style="background-color: #f4f7f6; min-height: 100vh;">
        ...
        <div class="row align-items-center mb-4">
            <div class="col-md-6">
                ...
                <p class="text-muted mb-0">Pantau arus kas <span class="badge bg-white text-primary shadow-sm">Cash</span> &
                    <span class="badge bg-white text-info shadow-sm">Bank</span> secara real-time.</p>
            </div>
            <div class="col-md-6 text-md-end mt-3 mt-md-0 d-flex gap-2 justify-content-md-end flex-wrap">
                ...
                <a href="{{ route('kasir.laporan-kas.index') }}"
                    ...
                    <i class="fas fa-file-invoice-dollar me-2"></i>Laporan Kas Harian
                </a>
        ...
        <div class="d-flex align-items-center mb-3">
            <div class="icon-box bg-warning text-white rounded-3 me-2 d-flex align-items-center justify-content-center"
                    ...
                    <div class="card border-0 shadow-sm h-100 card-hover" style="border-radius: 12px;">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-start">
                                <div>
                                    <span
                                        class="text-muted d-block small fw-bold text-uppercase mb-1">{{ $stat['label'] }}</span>
        ...
        <div class="d-flex align-items-center mb-3">
            <div class="icon-box bg-info text-white rounded-3 me-2 d-flex align-items-center justify-content-center"
                style="width: 32px; height: 32px;">
        ...
        <div class="row g-3 mb-5">
            ...
            @foreach ($bankStats as $stat)
                <div class="col-xl-3 col-md-6">
                    <div class="card border-0 shadow-sm h-100 card-bank border-bottom border-3 border-{{ $stat['color'] }}"
                                    ...
                                    @foreach ($pecahanKas as $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <span
                                                    class="badge rounded-pill bg-light text-dark border px-3">{{ $item->jumlah }}</span>
                                            </td>
                                ...
                                <tbody>
                                    @forelse ($riwayatTransaksi as $trx)
                                        ...
                                        <tr class="hover-row">
                                            <td class="ps-4">
                                                <span
                                                    class="text-muted small fw-bold">{{ $trx->created_at->format('H:i') }}</span>
                                            </td>
                                            <td>
                                                <span
                                                    class="badge {{ $isCash ? 'bg-soft-warning text-warning' : 'bg-soft-info text-info' }} rounded-pill px-3">
                                                    <i
                                                        class="fas {{ $isCash ? 'fa-money-bill' : 'fa-credit-card' }} me-1 small"></i>
                                                    {{ $isCash ? 'CASH' : 'BANK' }}
                                                </span>
                                            </td>
                                            <td>
                                                <div class="d-flex flex-column">
                                                    ...
                                                    <small class="text-muted text-uppercase"
                                                        style="font-size: 10px;">{{ $trx->metode_pembayaran }}</small>
                                                </div>
                                            </td>
                                            <td class="text-end pe-4 fw-bold {{ $isIn ? 'text-success' : 'text-danger' }}">
                                                <span class="small">{{ $isIn ? '+' : '-' }}</span> Rp
