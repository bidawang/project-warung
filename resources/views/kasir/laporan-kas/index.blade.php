...
@section('content')
    ...
    {{-- ===================== MODAL KAS ===================== --}}
    <div class="modal fade" id="modalInputKas" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 shadow-lg" style="border-radius:20px;">
                <form action="{{ route('kasir.laporan-kas.store') }}" method="POST">
                    @csrf

                    <div class="modal-header">
                        <h5 class="fw-bold">Audit Kas Fisik</h5>
                    </div>

                    <div class="modal-body">
                        <p class="text-muted small mb-3">
                            Masukkan jumlah uang fisik sesuai pecahan.
                        </p>

                        <div class="row g-3">
                            @foreach ([100000, 75000, 50000, 20000, 10000, 5000, 2000, 1000, 500, 200, 100] as $pecahan)
                                <div class="col-md-4">
                                    <label class="small fw-bold">
                                        Rp {{ number_format($pecahan, 0, ',', '.') }}
                                    </label>
                                    <input type="number" name="pecahan[{{ $pecahan }}]" class="form-control"
                                        min="0" value="0">
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-light" data-bs-dismiss="modal">Batal</button>
                        <button class="btn btn-primary fw-bold">Simpan Audit</button>
                    </div>
                </form>
            </div>
        </div>
