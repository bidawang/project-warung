            ...
            <div class="card shadow-sm border-0 rounded-3">
                ...
                <div class="card-body p-4">
                    <form id="formJualPulsa" action="{{ route('kasir.pulsa.jual.store') }}" method="POST">
                        ...
                        {{-- JENIS PULSA --}}
                        <div class="mb-4">
                            <label class="form-label fw-semibold text-secondary small uppercase">1. Jenis Layanan</label>
...
<input type="hidden" name="nomor_hp" value="0">
                        ...
                        <div class="mb-4" id="nominal_wrapper" style="display: none;">
                            <label class="form-label fw-semibold text-secondary small uppercase">2. Pilih Nominal</label>
                            <select id="harga_pulsa_id" name="harga_pulsa_id" class="form-select select2" required disabled>
                                <option value="">-- Pilih Nominal --</option>
                                @foreach ($harga_pulsas as $p)
                                    <option value="{{ $p->id }}" 
                                        data-jual="{{ $p->harga_jual }}"
                                        data-hutang="{{ $p->harga_hutang }}"
                        ...
                        <div class="row bg-light rounded-3 p-3 mb-4 mx-0 border">
                            ...
                            <div class="col-sm-6 mb-3 mb-sm-0">
                                ...
                                <select id="jenis_pembayaran" name="jenis_pembayaran" class="form-select form-select-sm" required>
                        ...
                        <div class="mb-4" id="pelanggan_field" style="display:none">
                            ...
                            <select id="pelanggan_id" name="pelanggan_id" class="form-select select2">
                                ...
                                @foreach ($pelanggans as $p)
                                    <option value="{{ $p->id }}">{{ $p->name }} ({{ $p->nomor_hp }})</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div id="tunai_area">
                            <div class="row">
                                <div class="col-6 mb-4">
                                    <label class="form-label fw-semibold text-secondary small uppercase">Uang Bayar</label>
                                    <input type="number" id="bayar" name="bayar" class="form-control" value="0">
                                </div>
                                <div class="col-6 mb-4">
                                    <label class="form-label fw-semibold text-secondary small uppercase">Kembalian</label>
                                    <input type="text" id="kembalian_display" class="form-control bg-light border-0 fw-bold" value="Rp 0" readonly>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-grid gap-2">
                            ...
                            <a href="{{ route('kasir.pulsa.index') }}" class="btn btn-link text-muted text-decoration-none small">
...
<script>
    $(document).ready(function() {
        ...
        $jenisPulsa.on('change', function() {
            ...
            $nomorHP.attr('placeholder', tipe === 'listrik' ? 'Contoh: 12345678901' : 'Contoh: 081234567890');
        ...
        function hitung() {
            ...
            if (selectedNominal.val()) {
                // Gunakan harga jual atau harga hutang berdasarkan metode bayar
                harga = (metode === 'hutang pulsa') 
...
<style>
    ...
    .uppercase { letter-spacing: 0.05em; font-size: 0.65rem; }
</style>
@endsection