        ...
        <div class="col-lg-5 col-xl-4 order-1 order-lg-2">
            ...
            <div class="mb-3">
                <div class="px-2 d-flex justify-content-between align-items-end mb-2">
                    ...
                    <small class="text-muted" style="font-size: 10px;">{{ count($saldoPulsas) }} Provider</small>
                </div>
                    ...
                    @forelse ($saldoPulsas as $pulsa)
                        <div class="card border-0 shadow-sm rounded-4 me-2 mb-lg-2 flex-shrink-0 flex-lg-shrink-1" 
                            ...
                            <div class="card-body p-3 text-white">
                                {{-- Mengambil nama jenis melalui relasi hargaPulsa --}}
                                <small class="text-white-50 d-block fw-bold text-uppercase" style="font-size: 9px;">
                                    {{ $pulsa->hargaPulsa->jenisPulsa->nama_jenis ?? 'PROVIDER' }}
                                </small>
                            ...
                            <tbody class="small">
                                @forelse ($transaksi_pulsa as $trx)
                                    <tr class="border-bottom">
                                        <td class="ps-3 py-2">
                                            <div class="fw-bold text-dark">Pulsa {{ number_format($trx->hargaPulsa->jumlah_pulsa ?? 0, 0, ',', '.') }}</div>
                                            <div class="text-muted" style="font-size: 10px;">{{ $trx->created_at->diffForHumans() }}</div>
                                        </td>
                                        <td>
                                            <span class="fw-bold text-primary">Rp {{ number_format($trx->total ?? 0, 0, ',', '.') }}</span>
                                        </td>
                                        <td class="text-end pe-3">
                                            <span class="badge rounded-pill border fw-bold text-dark bg-light" style="font-size: 9px;">
                        ...
                        <table class="table align-middle mb-0 table-hover" id="hargaTable">
                            <thead class="bg-light">
                                <tr class="fs-7 text-muted text-uppercase">
                                    ...
                                    <th>Harga Jual</th>
                                    <th class="text-center pe-4">Provider</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($harga_pulsas as $harga)
                                    <tr class="harga-item" 
                                        ...
                                        data-operator="{{ strtolower($harga->jenisPulsa->nama_jenis ?? '') }}">
                                        <td class="ps-4 fw-bold text-dark">Pulsa {{ number_format($harga->jumlah_pulsa, 0, ',', '.') }}</td>
                                        <td class="fw-bold text-primary">Rp {{ number_format($harga->harga_jual, 0, ',', '.') }}</td>
                                        <td class="text-center pe-4">
                                            <span class="badge bg-soft-info text-info rounded-pill px-3">
                                                {{ $harga->jenisPulsa->nama_jenis ?? 'Umum' }}
                                            </span>
                                        </td>
                                    </tr>
                                @endforeach
                                ...
                                <div>
                                    <div class="d-flex align-items-center mb-1">
                                        ...
                                        <span class="badge bg-light text-dark border" style="font-size: 9px;">
                                            {{ strtoupper($harga->jenisPulsa->nama_jenis ?? 'UMUM') }}
                                        </span>
                                    </div>
                                    <small class="text-muted">Jual Cepat</small>
                                </div>
                                <div class="text-end">
                                    <div class="fw-bold text-primary fs-5">Rp {{ number_format($harga->harga_jual, 0, ',', '.') }}</div>
                                    <small class="text-success fw-bold" style="font-size: 9px;">Tersedia <i class="fas fa-check-circle"></i></small>
                                </div>
                            </div>
                        @empty
...
<style>
    ...
    .harga-item:active { background-color: #f8f9fa; }
</style>
