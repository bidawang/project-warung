        ...
        <div class="col-12 col-sm-6 col-md-6 col-lg-4 d-flex justify-content-center product-item"
            ...
            <div class="card product-card shadow-sm border-0 rounded-3" style="width: 350px;">
                <div class="card-body p-2 d-flex align-items-center gap-3">
                    ...
                    {{-- Konten di kanan (harga, stok, tombol) --}}
    ...
    @endforelse
        ...
        <div class="col-md-4">
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <form id="checkoutForm" action="{{ route('barangkeluar.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="number" class="form-control" id="payment" placeholder="Jumlah uang pembayaran" min="0">
                        </div>
                        <div class="form-group mb-3">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function calculateHargaTotal(kuantitasList, hargaJualDasar, qty) {
            ...
            // Urutkan kuantitas descending berdasarkan jumlah untuk mengutamakan bundle terbesar
            ...
            for (const bundle of kuantitasList) {
                ...
                if (bundleQty <= 0) continue; // Pastikan kuantitas bundle valid
            ...
            // Hitung sisa kuantitas dengan harga jual dasar jika tidak masuk bundle
            if (sisaQty > 0) {
        ...
        function updateCart() {
            ...
            for (const id in cart) {
                const item = cart[id];
                total += item.totalPrice; // Tambahkan total harga item yang sudah dihitung
            ...
            calculateChange(); // Hitung kembalian setelah total diperbarui
            checkoutBtn.disabled = Object.keys(cart).length === 0; // Aktifkan/nonaktifkan tombol checkout
        }
        ...
        function calculateChange() {
            ...
            // Tombol checkout disabled jika kembalian negatif atau keranjang kosong
            checkoutBtn.disabled = change < 0 || Object.keys(cart).length === 0;
        }
        ...
        document.body.addEventListener('click', function(e) {
            if (e.target.classList.contains('add-to-cart-btn')) {
                ...
                const stokId = productCard.dataset.stokId; // Menggunakan stok-id sebagai kunci unik
                const name = productCard.dataset.name;
                const maxStock = parseInt(productCard.dataset.stock);
                const hargaJualDasar = parseInt(productCard.dataset.hargaJualDasar);
                const kuantitasList = JSON.parse(productCard.dataset.kuantitasList);

                if (!cart[stokId]) { // Jika item belum ada di keranjang
                    if (maxStock < 1) {
                        alert('Stok produk ini sudah habis.');
                        return;
                    }

                    // Tentukan harga per unit pertama kali ditambahkan (bukan harga total bundle)
                    // Ini akan menjadi harga yang ditampilkan di <small> keranjang
                    let unitPriceDisplay = hargaJualDasar;
                    // Cek apakah ada bundle untuk 1 unit, jika ada gunakan harganya
                    ...
                    cart[stokId] = {
                        stokId: stokId,
                        name: name,
                        unitPrice: unitPriceDisplay, // Harga per satuan untuk tampilan
                        kuantitasList: kuantitasList,
                        hargaJualDasar: hargaJualDasar,
                        quantity: 1,
                        totalPrice: calculateHargaTotal(kuantitasList, hargaJualDasar, 1) // Hitung total untuk 1 unit
                    };
                    ...
                    // Hitung ulang total harga item sesuai quantity baru
        ...
        document.body.addEventListener('click', function(e) {
            if (e.target.classList.contains('remove-from-cart-btn')) {
                // Cari stokId dari elemen parent <li>
                ...
                if (cart[stokId]) {
                    if (cart[stokId].quantity > 1) {
                        ...
                        delete cart[stokId]; // Hapus item jika kuantitas jadi 0
                    }
                }
                updateCart();
            }
        ...
        // Panggil updateCart pertama kali untuk memastikan tampilan awal benar (misal: jika ada item dari sesi sebelumnya, dll)
        updateCart();
    });
</script>
@endsection