...
<div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
    ...
    @forelse ($barangs as $br)
        ...
        <label
            class="group relative flex flex-col bg-white border rounded-xl shadow-sm transition-all cursor-pointer overflow-hidden
{{ in_array((int) $br->id, array_map('intval', $selected), true) ? 'border-blue-500 ring-2 ring-blue-100 ring-inset' : 'border-gray-100 hover:border-blue-300 hover:shadow-md' }}            {{ $isUsedElsewhere ? 'border-l-4 border-l-amber-400' : '' }}">

            <div class="p-3 flex-1 flex flex-col">
                {{-- Checkbox + Nama --}}
                <div class="flex items-start gap-2 mb-2">
                    <input type="checkbox" name="barangs[]" value="{{ $br->id }}"
                        class="mt-1 form-checkbox h-4 w-4 text-blue-600 rounded border-gray-300 transition focus:ring-blue-500"
                        {{-- Ini yang memastikan checkbox tercentang jika ID ada di array $selected --}} @checked(in_array((int) $br->id, array_map('intval', $selected), true))>
                        {{ in_array((int) $br->id, array_map('intval', $selected), true) ? 'TRUE' : 'FALSE' }}
                    ...
                    <div class="mt-2 pt-2 border-t border-amber-50">
                        ...
                        <div class="flex flex-wrap gap-1">
                            @foreach ($otherAreas as $areaName)
                                <span
                                    class="bg-amber-100 text-amber-800 text-[8px] px-1.5 py-0.5 rounded-md font-medium">
                                    {{ $areaName }}
                                </span>
                            @endforeach
                        </div>
            ...
            @if (in_array((int) $br->id, array_map('intval', $selected), true))
