    ...
    <div class="p-4 bg-gray-50 min-h-screen">
        ...
        <form action="{{ route('admin.asalbarang.store') }}" method="POST">
            ...
            <div class="bg-white p-5 rounded-xl border border-gray-100 shadow-sm mb-6">
                <label class="block text-sm font-bold text-gray-700 mb-2">Area Pembelian <span
                        class="text-red-500">*</span></label>
                <select name="id_area_pembelian" id="areaSelect"
                    class="w-full md:w-1/3 border-gray-300 rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 shadow-sm transition"
                    required>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div>
                        ...
                        <select id="kategoriSelect"
                            class="w-full border-gray-300 rounded-lg p-2 shadow-sm focus:ring-blue-500">
            ...
            {{-- FLOATING ACTION BAR --}}
            <div id="saveButton"
                class="{{ $areaId ? 'flex' : 'hidden' }} fixed bottom-0 left-0 right-0 md:left-64 bg-white border-t border-gray-200 p-4 justify-end shadow-[0_-4px_10px_rgba(0,0,0,0.05)] z-10">
                <button type="submit"
                    class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-10 rounded-full shadow-lg transition transform hover:scale-105 active:scale-95">
                    Simpan Perubahan
                </button>
            </div>
        </form>
    ...
    <script>
        ...
        kategoriSelect.addEventListener('change', function() {
            ...
            if (selectedKategoriId) {
                const filtered = allSubkategoris.filter(sub => sub.id_kategori === selectedKategoriId);
