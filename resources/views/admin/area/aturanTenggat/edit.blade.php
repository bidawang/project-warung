...
<div class="flex-1 flex flex-col overflow-hidden">
    <header class="flex justify-between items-center p-6 bg-white border-b-2 border-gray-200">
        <h1 class="text-2xl font-bold text-gray-800">Edit Aturan Tenggat {{ $area->area ?? '' }}</h1>
    ...
    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6 md:p-10">
        <div class="bg-white shadow-md rounded-lg p-8">
            ...
            <form action="{{ route('admin.aturanTenggat.update', $aturanTenggat->id) }}" method="POST" class="w-full space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="number" min="1" max="31" name="tanggal_awal" value="{{ old('tanggal_awal', $aturanTenggat->tanggal_awal) }}"
                ...
                <div>
                    ...
                    <input type="number" min="1" max="31" name="tanggal_akhir" value="{{ old('tanggal_akhir', $aturanTenggat->tanggal_akhir) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2" required>
                </div>

                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Jatuh Tempo (Hari)</label>
                    <input type="number" min="0" name="jatuh_tempo_hari" value="{{ old('jatuh_tempo_hari', $aturanTenggat->jatuh_tempo_hari) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2" required>
                </div>

                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Bunga (%)</label>
                    <input type="number" step="0.01" min="0" name="bunga" value="{{ old('bunga', $aturanTenggat->bunga) }}"
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="3"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2">{{ old('keterangan', $aturanTenggat->keterangan) }}</textarea>
                </div>
