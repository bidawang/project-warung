...
@section('content')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="md:col-span-1 bg-white p-6 rounded-lg shadow-md h-fit sticky top-6">
                        ...
                        <div class="flex justify-around mb-4 space-x-2">
                            <button type="button" data-view="warung"
                                class="btn-view-toggle flex-1 bg-blue-600 text-white px-3 py-2 rounded-lg text-sm font-semibold transition-colors duration-200">
                                Berdasarkan Warung
                            </button>
                            <button type="button" data-view="barang"
                                class="btn-view-toggle flex-1 bg-gray-300 text-gray-800 px-3 py-2 rounded-lg text-sm font-semibold transition-colors duration-200">
                                Berdasarkan Barang
                            </button>
                        </div>

                        {{-- Input Search JS --}}
                        <input type="text" id="searchRencana" onkeyup="filterRencanaBelanja()"
                        ...
                        <div id="rencanaBelanjaContainer" class="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                            ...
                            <div id="viewWarung" class="view-content space-y-4">
                                @forelse ($rencanaBelanjaByWarung as $namaWarung => $items)
                                    <div class="item-block border p-3 rounded-lg bg-indigo-50"
                                        data-nama-utama="{{ $namaWarung }}">
                            ...
                            <div id="viewBarang" class="view-content space-y-4 hidden">
                                ...
                                <div id="viewBarangTotal" class="border p-4 rounded-lg bg-yellow-100 mb-6">
                                    <h3 class="font-bold text-lg text-yellow-800 mb-3 border-b pb-2">Total Kebutuhan Barang
                                    </h3>
                                    <ul class="list-none text-sm space-y-1" id="listTotalBarang">
                                        ...
                                        @isset($rencanaBelanjaTotalByBarang)
                                            @forelse ($rencanaBelanjaTotalByBarang as $namaBarang => $totalKebutuhan)
                                                <li class="flex justify-between items-center py-1 border-b border-yellow-200"
                                                    data-item-nama="{{ $namaBarang }}">
                                                    <span class="font-semibold">{{ $namaBarang }}</span>
                                                    <span class="text-yellow-700">{{ $totalKebutuhan }} pcs</span>
                                                </li>
                                            @empty
                                                <li>
                                                    <p class="text-gray-500">Tidak ada total kebutuhan barang.</p>
                                                </li>
                                            @endforelse
                                        @endisset
                                    </ul>
                                ...
                                @forelse ($rencanaBelanjaByBarang as $namaBarang => $items)
                                    <div class="item-block border p-3 rounded-lg bg-green-50"
                                        data-nama-utama="{{ $namaBarang }}">
                    ...
                    <div class="md:col-span-2 bg-white p-6 rounded-lg shadow-md">
                        ...
                        <form action="{{ route('admin.transaksibarang.store') }}" method="POST" id="formTransaksiBarang">
                                ...
                                <div id="areaContainer" class="space-y-6">
                                    ...
                                    <div class="area-block border rounded-lg p-4 bg-gray-50">
                                        <div class="flex justify-between items-center mb-3">
                                            <h3 class="font-semibold text-gray-700">Area Pembelian <span
                                                    class="area-index-label">1</span></h3>
                                            <button type="button"
                                                class="btn-remove-area text-red-600 hover:text-red-800 font-bold text-xl leading-none">&times;</button>
                                        </div>
                                        <select name="id_area[]"
                                            class="select-area w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 mb-4"
                                            required>
                                        ...
                                        <div>
                                            ...
                                            <table class="w-full border-collapse border mb-3 barangTable">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <select name="id_barang[0][]"
                                                            class="select-barang w-full border rounded-lg px-2 py-1"
                                                            required>
                                                            <option value="">Pilih Area dulu</option>
                                                            <!-- Opsi barang akan diisi secara dinamis via JS -->
                                                        </select>
                                                        <td class="border px-2 py-1">
                                                            <input type="number" name="jumlah[0][]"
                                                                class="input-jumlah w-full border rounded-lg px-2 py-1"
                                                                min="1" required />
                                                        </td>
                                                        <td class="border px-2 py-1">
                                                            <input type="text" name="total_harga[0][]"
                                                                class="total-harga w-full border rounded-lg px-2 py-1"
                                                                required />
                                                        </td>
                                                        <td class="border px-2 py-1">
                                                            <input type="date" name="tanggal_kadaluarsa[0][]"
                                                                class="input-tgl-kadaluarsa w-full border rounded-lg px-2 py-1" />
                                                        </td>
                                                        <td class="border px-2 py-1 text-center">
                                                            <button type="button"
                                                                class="btn-remove-row text-red-600 hover:text-red-800 font-bold text-xl leading-none">&times;</button>
                                                        </td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                            <button type="button"
                                                class="btn-add-row bg-gray-200 hover:bg-gray-300 px-3 py-1 rounded-lg text-sm text-gray-700">+
                                                Tambah Barang</button>
                                        </div>
                                    </div>
                                </div>

                                <button type="button" id="btnAddArea"
                                    class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg mt-4 font-semibold">
                            ...
                            {{-- Keterangan --}}
                            <div class="mb-6">
                                <label for="keterangan" class="block text-gray-700 font-semibold mb-2">Keterangan
                                    (Opsional)</label>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const barangByArea = @json($barangByArea);
            ...
            function updateBarangOptions(areaBlock) {
                const selectArea = areaBlock.querySelector('.select-area');
                const selectBarangList = areaBlock.querySelectorAll(
                    '.select-barang'); // Semua select-barang di area ini
                const selectedArea = selectArea.value;

                selectBarangList.forEach(selectBarang => {
                    // Hancurkan Select2 sebelum update
                    ...
                    // Tambahkan opsi baru berdasarkan area
                    if (selectedArea && barangByArea[selectedArea]) {
                        barangByArea[selectedArea].forEach(item => {
            ...
            window.filterRencanaBelanja = function() {
                ...
                document.querySelectorAll('.view-content:not(.hidden) .item-block').forEach(block => {
                    ...
                    block.querySelectorAll('li').forEach(listItem => {
                        const itemNama = listItem.dataset.itemNama
                            .toLowerCase(); // Barang atau Warung
            ...
            function reindexAreaBlocks() {
                areaContainer.querySelectorAll('.area-block').forEach((areaBlock, index) => {
                    ...
                    areaBlock.querySelectorAll(
                        'select[name^="id_barang"], input[name^="jumlah"], input[name^="total_harga"], input[name^="tanggal_kadaluarsa"]'
                    ).forEach(el => {
                        if (el.name.includes('id_barang')) el.name = `id_barang[${index}][]`;
                        else if (el.name.includes('jumlah')) el.name = `jumlah[${index}][]`;
                        else if (el.name.includes('total_harga')) el.name =
                            `total_harga[${index}][]`;
                        else if (el.name.includes('tanggal_kadaluarsa')) el.name =
                            `tanggal_kadaluarsa[${index}][]`;
                    });
                });
            ...
            document.addEventListener('change', e => {
                if (e.target.classList.contains('select-area')) {
                    updateAreaOptions();
                    // Tambahkan: Update opsi barang di area yang sama
                    const areaBlock = e.target.closest('.area-block');
                    updateBarangOptions(areaBlock);
            ...
            document.addEventListener('click', e => {
                ...
                if (e.target.classList.contains('btn-add-row')) {
                    ...
                    newRow.querySelectorAll('select, input').forEach(el => {
                        if (el.name.includes('id_barang')) el.name = `id_barang[${areaIndex}][]`;
                            ...
                            `total_harga[${areaIndex}][]`;
                        else if (el.name.includes('tanggal_kadaluarsa')) el.name =
                            `tanggal_kadaluarsa[${areaIndex}][]`;

                        el.value = (el.tagName === 'SELECT') ? "" : (el.type === 'number' ? "" :
                            "");
                        if (el.classList.contains('total-harga')) el.value =
                            "0"; // Pastikan 0 atau kosong
                    });
                ...
                if (e.target.classList.contains('btn-view-toggle')) {
                    ...
                    document.getElementById('view' + targetView.charAt(0).toUpperCase() + targetView.slice(
