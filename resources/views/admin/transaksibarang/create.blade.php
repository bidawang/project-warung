            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <template x-for="(items, barang) in rencanaByBarang">
                                <div x-show="barang.toLowerCase().includes(search.toLowerCase())"
                                    ...
                                    <h3 class="font-bold text-green-700">
                                        ...
                                        <span class="text-xs bg-green-200 text-green-800 px-2 py-0.5 rounded ml-2">
                                            Total: <span x-text="totalBarang(items)"></span>
                                        </span>
                                    </h3>
                                    <ul class="text-sm list-disc list-inside">
                                        <template x-for="i in items">
                ...
                <section class="lg:col-span-2 bg-white rounded-xl shadow p-6">
                    <form method="POST" action="{{ route('admin.transaksibarang.store') }}">
                                        ...
                                        <template x-for="(item, iIndex) in area.items" :key="iIndex">
                                            <tr>
                                                ...
                                                <td class="border p-1 relative">
                                                    ...
                                                    <div x-show="item.open" @click.outside="item.open=false"
                                                        class="absolute z-50 bg-white border rounded shadow mt-1 w-full max-h-56 overflow-y-auto">
                                                        <input x-model="item.searchBarang" placeholder="Cari barang..."
                                                            class="w-full px-2 py-1 border-b">
                                                        <template
                                                            x-for="b in (barangByArea[area.area_id] ?? [])
                                                ...
                                                <td class="border p-1">
                                                    <input type="number" min="1" x-model.number="item.jumlah"
                                                        @input="hitungTotal(aIndex,iIndex)" :name="`jumlah[${aIndex}][]`"
                                                        class="w-full border rounded px-2 py-1">
                                                </td>

                                                {{-- TOTAL (EDITABLE) --}}
                                                <td class="border p-1">
                                                    <input type="number" min="0" x-model.number="item.harga"
                                                ...
                                                <td class="border p-1 text-center">
                                                    <button type="button" @click="removeItem(aIndex,iIndex)"
                                                        class="text-red-600 font-bold">×</button>
                                                </td>
                                            </tr>
                                        </template>
                        ...
                        <button type="button" @click="addArea()" class="bg-green-600 text-white px-4 py-2 rounded-lg">
                            + Tambah Area
                        </button>
