                        ...
                        <tbody>
                            @forelse ($aturanTenggats as $aturan)
                                <tr class="border-b hover:bg-indigo-50 transition duration-150">
                                    ...
                                    <td class="py-3 px-4 text-center font-extrabold text-red-600">{{ $aturan->bunga }}%
                                    </td>
...
<form action="{{ route('admin.aturanTenggat.destroy', $aturan->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Aturan Tenggat ini? Tindakan ini tidak dapat dibatalkan.');">
    @csrf
    @method('DELETE') {{-- Wajib ada untuk memalsukan metode HTTP menjadi DELETE --}}

    <button type="submit"
        class="text-red-600 hover:text-red-800 font-semibold text-sm hover:underline p-0 m-0 border-none bg-transparent cursor-pointer ml-2 focus:outline-none focus:ring-0">
        <i class="fas fa-trash"></i> Hapus
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-6 text-gray-500">Belum ada aturan tenggat yang
                                        ditambahkan.</td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <form action="{{ route('admin.hutang.index') }}" method="GET" id="filter-form">
                    ...
                    <div class="flex flex-wrap items-end gap-4">
                        ...
                        <div class="flex items-center h-full pt-6">
                            ...
                            <label for="expired" class="ml-2 block text-sm font-medium text-gray-700">Hanya Jatuh
                                Tempo</label>
                        </div>

                        <div class="flex space-x-2 ml-auto">
                            <button type="submit"
                                class="bg-red-600 text-white px-4 py-2 rounded-lg shadow-md hover:bg-red-700 transition duration-150 font-semibold">
                                <i class="fas fa-filter mr-1"></i> Terapkan Filter
                            </button>
                            <a href="{{ route('admin.hutang.index') }}"
                                class="text-gray-500 hover:text-red-600 px-4 py-2 border border-gray-300 rounded-lg shadow-sm transition duration-150">
                                Reset
                            </a>
                        </div>
                    </div>
            ...
            @else
                ...
                <div id="container-compact"
                    class="flex overflow-x-auto pb-4 -mx-6 px-6 space-x-6 scrollbar-thin scrollbar-thumb-red-300 scrollbar-track-red-100"
                        ...
                        <div class="warung-container flex-shrink-0 w-80 bg-white shadow-xl border-t-8 rounded-xl flex flex-col transition-all duration-300
                            ...
                            <div class="p-4 border-b bg-gray-50">
                                <h2 class="text-lg font-bold text-gray-800 flex items-center mb-1">
                                    <i
                                        class="fas fa-store mr-2 {{ $isExpired ? 'text-yellow-600' : 'text-indigo-600' }}"></i>
                                    **{{ $dataWarung['nama_warung'] }}**
                                </h2>
                                <p class="text-xs text-gray-500 mb-3">Total {{ $dataWarung['hutang_list']->count() }}
                                    transaksi hutang</p>
                            ...
                            <div class="p-4 flex-1 overflow-y-auto max-h-[500px] space-y-3 custom-scrollbar"
                                id="hutang-list-{{ $dataWarung['id'] }}">
                                {{-- Item Hutang Compact --}}
                                @forelse ($dataWarung['hutang_list'] as $hutang)
                                    <a href="{{ route('admin.hutang.detail', $hutang->id) }}"
                                        ...
                                        <p
                                            class="font-bold text-lg {{ $hutang->jumlah_sisa_hutang > 0 ? 'text-red-700' : 'text-green-700' }}">
                                            Sisa: Rp. {{ number_format($hutang->jumlah_sisa_hutang, 0, ',', '.') }}
                                        </p>
                                        <div class="mt-1 text-xs space-x-2">
                                            <span
                                                class="px-2 py-0.5 rounded-full font-medium {{ $hutang->status === 'lunas' ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800' }}">
                                                {{ $hutang->status === 'lunas' ? 'LUNAS' : 'BELUM LUNAS' }}
                                            </span>
                                            @if ($hutang->tenggat && $hutang->tenggat->isPast() && $hutang->status === 'belum_lunas')
                                                <span
                                                    class="px-2 py-0.5 rounded-full bg-yellow-400 text-yellow-900 font-bold shadow-sm">JATUH
                                                    TEMPO</span>
                                            @endif
                                        </div>
                                    </a>
                                ...
                                @endforelse
                                <div class="not-found-message text-center py-6 text-yellow-600 font-semibold"
                                    style="display: none;">
                    ...
                    @empty
                        <div class="w-full bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-6 rounded-lg shadow-md -mx-2"
                            role="alert">
                            <p class="font-bold text-xl mb-1">Informasi</p>
                                ...
                                <tbody>
                                    @forelse ($hutangList as $hutang)
                                        <tr class="transaksi-item border-b hover:bg-red-50 transition duration-150"
                                            ...
                                            <td class="py-3 px-4 font-bold text-indigo-600">
                                                {{ $hutang->warung->nama_warung ?? 'N/A' }}</td>
                                            <td class="py-3 px-4 text-gray-700 font-medium">
                                                {{ $hutang->user->name ?? 'User Dihapus' }}</td>
                                            <td class="py-3 px-4 text-xs font-semibold whitespace-nowrap">
                                                @if ($hutang->tenggat)
                                                    <span
                                                        class="{{ $hutang->tenggat->isPast() && $hutang->jumlah_sisa_hutang > 0 ? 'text-red-500 font-bold' : 'text-gray-600' }}">
                                            ...
                                            <td class="py-3 px-4 text-right text-gray-800">
                                                {{ number_format($hutang->jumlah_hutang_awal, 0, ',', '.') }}</td>
                                            <td
                                                class="py-3 px-4 text-right font-extrabold whitespace-nowrap {{ $hutang->jumlah_sisa_hutang > 0 ? 'text-red-700' : 'text-green-700' }}">
                                            ...
                                            <td class="py-3 px-4 text-center">
                                                <a href="{{ route('admin.hutang.detail', $hutang->id) }}"
                                                    class="text-indigo-600 hover:text-indigo-800 font-semibold text-sm hover:underline">
                                    ...
                                    @empty
                                        <tr class="empty-row">
                                            <td colspan="7" class="text-center py-6 text-gray-500">Tidak ada hutang
                                                yang ditemukan dengan filter ini.</td>
                                        </tr>
                                    @endforelse
                                    <tr class="not-found-message" style="display: none;">
                                        <td colspan="7" class="text-center py-4 text-yellow-600 font-semibold">Hasil
                                            pencarian tidak ditemukan.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
