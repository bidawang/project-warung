    ...
    <div class="mt-8 flow-root">
        <div class="max-w-4xl mx-auto bg-white shadow-xl rounded-xl overflow-hidden">
            <div class="px-4 py-5 sm:p-6">
                <h2 class="text-xl font-semibold text-indigo-700 mb-4 border-b pb-2">Data Aturan #{{ $aturanTenggat->id ?? '...' }}</h2>

                {{-- Asumsi: $aturanTenggat adalah objek Aturan Tenggat yang sedang diedit --}}
                <form action="{{ route('admin.aturanTenggat.update', $aturanTenggat->id) }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-6">
                        ...
                        <select id="id_warung" name="id_warung" required
                            ...
                            @foreach ($warung as $item)
                                <option value="{{ $item->id }}" {{ old('id_warung', $aturanTenggat->id_warung) == $item->id ? 'selected' : '' }}>{{ $item->nama_warung }}</option>
                    ...
                    <div class="mt-8 border-t border-gray-200 pt-6">
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <input type="number" id="tanggal_awal" name="tanggal_awal" value="{{ old('tanggal_awal', $aturanTenggat->tanggal_awal) }}" min="1" max="31" placeholder="1-31" required
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <input type="number" id="tanggal_akhir" name="tanggal_akhir" value="{{ old('tanggal_akhir', $aturanTenggat->tanggal_akhir) }}" min="1" max="31" placeholder="1-31" required
                                    class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                @error('tanggal_akhir')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <input type="number" id="jatuh_tempo_hari" name="jatuh_tempo_hari" value="{{ old('jatuh_tempo_hari', $aturanTenggat->jatuh_tempo_hari) }}" min="1" placeholder="Misalnya 7" required
                                    class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <div class="mt-1 flex rounded-md shadow-sm">
                                    <input type="text" id="bunga" name="bunga" value="{{ old('bunga', $aturanTenggat->bunga) }}" placeholder="Contoh: 5" required
                    ...
                    <div class="mt-6 border-t border-gray-200 pt-6">
                        ...
                        <div class="mt-1">
                            <textarea id="keterangan" name="keterangan" rows="3"
                                class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md">{{ old('keterangan', $aturanTenggat->keterangan) }}</textarea>
                        </div>
                        @error('keterangan')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
