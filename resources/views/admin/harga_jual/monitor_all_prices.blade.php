            ...
            <div class="container mx-auto">
                ...
                <div class="bg-red-50 border-l-4 border-red-400 p-4 mb-6 rounded-md">
                    <p class="text-sm text-red-700 font-semibold">
                        ⚠️ Halaman ini memuat **SEMUA** harga barang di **SEMUA** warung aktif. Proses loading mungkin
                        memerlukan waktu jika data sangat banyak.
                    </p>
                </div>
                ...
                @forelse ($barangPricesByWarung as $item)
                    <div class="bg-white rounded-xl shadow-lg p-6 mb-8 break-inside-avoid-page">
                        <h2 class="text-2xl font-bold text-gray-900 mb-4 border-b pb-2">
                            ...
                            <span class="text-sm font-normal text-gray-500 ml-2">({{ $item->prices->count() }} Warung
                                Terdampak)</span>
                        </h2>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Warung
                                            (Area)</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Stok
                                        </th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Harga
                                            Modal</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Harga
                                            Jual Range</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Laba (%)
                                        </th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Laba
                                            (Rp)</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Aksi
                                        </th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($item->prices as $priceData)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td
                                                class="px-3 py-2 text-sm {{ $priceData->stok_saat_ini <= 0 ? 'text-red-600 font-bold' : '' }}">
                                            ...
                                            <td class="px-3 py-2 text-sm">
                                                @if ($priceData->harga_jual_range_awal > 0)
                                                    Rp {{ number_format($priceData->harga_jual_range_awal, 0, ',', '.') }}
                                                    - Rp
                                                    {{ number_format($priceData->harga_jual_range_akhir, 0, ',', '.') }}
                                                @else
                                            ...
                                            <td class="px-3 py-2 text-sm font-bold">
                                                <span
                                                    class="{{ str_contains($priceData->persentase_laba, '-') ? 'text-red-600' : 'text-green-600' }}">
                                                    {{ $priceData->persentase_laba }}
                                                </span>
                                            </td>
                                            <td class="px-3 py-2 text-sm font-bold">
                                                <span
                                                    class="{{ str_contains($priceData->laba_range, '-') ? 'text-red-600' : 'text-green-600' }}">
                                            ...
                                            <td class="px-3 py-2 text-sm">
                                                ...
                                                <button type="button"
                                                    ...
                                                    data-warung-id="{{ $priceData->id_warung }}"
                ...
                @empty
                    <div class="text-center py-10 bg-white rounded-xl shadow-lg">
                        <p class="text-xl text-gray-500">Tidak ada barang yang memiliki data harga jual atau stok di warung
                            manapun saat ini.</p>
                    </div>
                @endforelse
                {{-- End Loop per Barang --}}
            ...
            <form id="updatePriceForm" action="{{ route('admin.harga_jual.update') }}" method="POST">
                ...
                <div class="mb-4">
                    <p class="text-sm text-gray-600">
                        <span class="font-semibold">Barang:</span> <span id="modal_barang_name"
                            class="text-indigo-600"></span><br>
                ...
                <div class="mb-4">
                    <label for="harga_jual_range_awal" class="block text-sm font-medium text-gray-700">Harga Jual Range Awal
                        (Rp)</label>
                ...
                <div class="mb-6">
                    <label for="harga_jual_range_akhir" class="block text-sm font-medium text-gray-700">Harga Jual Range
                        Akhir (Rp)</label>
                ...
                <div class="flex justify-end space-x-3">
                    <button type="button"
                        class="modal-close-btn inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none">
                        Batal
                    </button>
                    <button type="submit"
                        class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.edit-price-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    document.getElementById('modal_harga_modal').textContent =
                        `Rp ${new Intl.NumberFormat('id-ID').format(modalPrice)}`;
