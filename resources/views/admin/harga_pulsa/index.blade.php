...
@section('content')
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="flex justify-between items-center p-6 bg-white border-b-2 border-gray-200">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Manajemen Harga Pulsa</h1>
            </div>
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6 md:p-10">
            <div class="container mx-auto">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                    <h1 class="text-3xl font-bold text-gray-800 mb-4 md:mb-0">Daftar Harga Pulsa</h1>
                    <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 w-full md:w-auto">
                        {{-- Tombol Tambah Harga Pulsa --}}
                        ...
                        {{-- Form Search Bar --}}
                        <form action="{{ route('admin.harga-pulsa.index') }}" method="GET" class="relative w-full sm:w-auto">
                ...
                <div class="bg-white shadow-md rounded-lg overflow-hidden overflow-x-auto">
                    <table class="min-w-full leading-normal">
                        ...
                        <tbody>
                            @forelse ($hargaPulsas as $hargaPulsa)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-5 py-5 border-b border-gray-200 text-sm">
                                        {{-- Format sebagai mata uang (misal: 10.000) --}}
                                        <p class="text-gray-900 whitespace-no-wrap">Rp{{ number_format($hargaPulsa->jumlah_pulsa, 0, ',', '.') }}</p>
                                    </td>
                                    <td class="px-5 py-5 border-b border-gray-200 text-sm">
                                        <p class="text-gray-900 whitespace-no-wrap">Rp{{ number_format($hargaPulsa->harga, 0, ',', '.') }}</p>
                                    </td>
                                    <td class="px-5 py-5 border-b border-gray-200 text-sm">
                                        {{-- Tampilkan tanggal dengan format yang lebih mudah dibaca --}}
                                        <p class="text-gray-900 whitespace-no-wrap">{{ $hargaPulsa->created_at->format('d/m/Y H:i') }}</p>
                                    </td>
                                    <td class="px-5 py-5 border-b border-gray-200 text-sm text-right">
                                        ...
                                        {{-- Tombol Hapus --}}
                                        <form action="{{ route('admin.harga-pulsa.destroy', $hargaPulsa->id) }}" method="POST" class="inline-block ml-2" onsubmit="return confirm('Apakah Anda yakin ingin menghapus harga pulsa ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-5 py-5 border-b border-gray-200 text-center text-sm text-gray-500">
                                        Tidak ada data harga pulsa yang ditemukan.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                {{-- Pagination --}}
                <div class="mt-8">
                    {{-- Di sini Anda bisa menambahkan Blade directive untuk pagination jika menggunakan paginate() --}}
                    {{-- Contoh: {{ $hargaPulsas->links() }} --}}
                </div>
            </div>
        </main>
    </div>
