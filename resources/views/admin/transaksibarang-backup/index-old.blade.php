                    ...
                    <div id="rencanaContainer" class="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                        ...
                        <div id="viewWarung" class="view-content space-y-3">
                            @forelse($rencanaBelanjaByWarung as $warung => $items)
                                <div class="p-4 rounded-lg bg-indigo-50 border border-indigo-200 item-block shadow-sm"
                                    data-nama="{{ $warung }}">
                                    ...
                                    <ul class="list-disc list-inside text-sm text-gray-700 space-y-0.5 ml-2">
                                        @foreach ($items as $i)
                                            <li data-sub="{{ $i->barang->nama_barang }}">
                                                {{ $i->barang->nama_barang }}: <span
                                                    class="font-semibold">{{ $i->jumlah_awal - $i->jumlah_dibeli }}
                                                    pcs</span>
                                            </li>
                                        @endforeach
                                    </ul>
                        ...
                        <div id="viewBarang" class="view-content space-y-3 hidden">
                            <div id="totalKebutuhan"
                                class="p-4 rounded-lg bg-yellow-100 border border-yellow-200 mb-4 shadow-sm">
                                <h3 class="font-bold text-yellow-800 text-base mb-2 border-b border-yellow-300 pb-1">Total
                                    Kebutuhan Barang</h3>
                                <ul class="text-sm space-y-1">
                                    @forelse($rencanaBelanjaTotalByBarang ?? [] as $barang=>$total)
                                        <li data-sub="{{ $barang }}"
                                            class="flex justify-between items-center text-yellow-900">
                                            <span>{{ $barang }}</span>
                            ...
                            @forelse($rencanaBelanjaByBarang as $barang=>$items)
                                <div class="p-4 rounded-lg bg-green-50 border border-green-200 item-block shadow-sm"
                                    data-nama="{{ $barang }}">
                                    <h3 class="font-bold text-green-700 text-base mb-1">{{ $barang }}</h3>
                                    <ul class="list-disc list-inside text-sm text-gray-700 space-y-0.5 ml-2">
                                        @foreach ($items as $i)
                                            <li data-sub="{{ $i->warung->nama_warung }}">
                                                {{ $i->warung->nama_warung }}: <span
                                                    class="font-semibold">{{ $i->jumlah_awal - $i->jumlah_dibeli }}
                                                    pcs</span>
                                            </li>
                                        @endforeach
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            <div class="flex space-x-2 items-center delivery-row" data-trx="${trxId}" data-row-id="${rowId}">
                <select name="transaksi[${trxId}][details][${rowId}][id_warung]" required class="warung-select border border-gray-300 rounded px-2 py-1 text-sm flex-1 focus:ring-blue-500 focus:border-blue-500 bg-white transition duration-150">
                    ${baseOptions}
                </select>
        ...
        `;
            ...
            const viewW = document.getElementById('viewWarung'),
                viewB = document.getElementById('viewBarang');
            ...
            function toggleView(btn) {
                ...
                search.placeholder = view === 'warung' ? "Cari Warung atau Barang..." :
                    "Cari Barang atau Warung...";
                filterRencana();
            }
