...
@section('content')
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6 md:p-10">
            <div class="container mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <table class="min-w-full leading-normal">
                                <thead class="bg-gray-100">
                                    <tr>
                                        ...
                                        @endif
                                        <th
                                            class="px-4 py-3 border-b-2 text-left text-xs font-semibold text-gray-600 uppercase w-1/5">
                                            Barang</th>
                                        <th
                                            class="px-4 py-3 border-b-2 text-center text-xs font-semibold text-gray-600 uppercase w-[10%]">
                                            Sisa Jml.</th>
                                        <th
                                            class="px-4 py-3 border-b-2 text-left text-xs font-semibold text-gray-600 uppercase w-2/5">
                                            Tujuan & Jumlah Kirim</th>
                                        <th
                                            class="px-4 py-3 border-b-2 text-left text-xs font-semibold text-gray-600 uppercase w-[10%]">
                                            Harga Satuan</th>
                                        <th
                                            class="px-4 py-3 border-b-2 text-center text-xs font-semibold text-gray-600 uppercase w-[10%]">
                                            Aksi</th>
                                        ...
                                        <tr class="hover:bg-gray-50 border-t border-gray-200" id="trx-{{ $trx->id }}">
                                            @if ($status === 'pending')
                                                <td class="px-4 py-4 border-b text-sm text-center">
                                                    <input type="checkbox" class="chk-trx cursor-pointer"
                                                        data-id="{{ $trx->id }}" name="transaksi_ids[]"
                                                        value="{{ $trx->id }}" data-valid="false">
                                                </td>
                                            @endif
                                            ...
                                            <td class="px-4 py-4 border-b text-sm text-center">
                                                ...
                                                <span class="font-bold text-lg text-blue-600" id="sisa-{{ $trx->id }}"
                                                    data-max="{{ $trx->jumlah }}">{{ $trx->jumlah }}</span>
                                            </td>

                                            <td class="px-4 py-4 border-b text-sm">
                                                {{-- Container untuk input pengiriman Warung & Qty (Awalnya kosong) --}}
                                                <div id="deliveries-{{ $trx->id }}" data-id="{{ $trx->id }}"
                                                    class="space-y-2"></div>
                                            ...
                                            <td class="px-4 py-4 border-b text-sm text-gray-600">
                                                {{ number_format($trx->harga, 0, ',', '.') }}</td>

                                            <td class="px-4 py-4 border-b text-sm text-center">
                                                @if ($status === 'pending')
                                                    <button type="button"
                                                        class="btn-add text-xs bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-1 px-3 rounded-full transition duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="{{ $status === 'pending' ? 6 : 5 }}"
                                                class="px-5 py-5 border-b text-center text-gray-500">
                ...
                <div class="md:col-span-1 bg-white p-6 rounded-xl shadow-xl h-fit sticky top-6 border border-gray-100">
                    <form id="formKirimRencana" method="POST"
                        action="{{ route('admin.transaksibarang.kirim.rencana.proses') }}">
                        ...
                        <input type="text" id="searchRencana"
                            class="w-full mb-4 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div id="rencanaContainer" class="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                            @forelse($rencanaBelanjaByWarung as $warungId => $items)
                                <div class="p-4 rounded-lg bg-indigo-50 border border-indigo-200 item-block rencana-warung-block"
                                    ...
                                    <h3
                                        class="font-bold text-indigo-700 text-base mb-2 border-b border-indigo-300 pb-1 flex justify-between items-center">
                                        <span>{{ $items[0]->warung->nama_warung }}</span>
                                        <input type="checkbox" class="chk-rencana-warung cursor-pointer"
                                            data-warung-id="{{ $warungId }}" data-valid="false" />
                                    </h3>
                                    <ul class="list-none text-sm text-gray-700 space-y-2">
                                        @foreach ($items as $i)
                                            ...
                                            <li class="rencana-item flex flex-col space-y-1 p-2 border-l-4 border-indigo-400 bg-white shadow-sm"
                                                ...
                                                data-kebutuhan="{{ $jumlahKebutuhan }}">
                                                <span class="font-semibold">{{ $namaBarang }}</span>
                                                <div class="flex items-center space-x-2">
                                                    <span class="text-xs text-gray-500 w-16">Kebutuhan:</span>
                                                    <span
                                                        class="font-bold text-sm text-red-600 w-10">{{ $jumlahKebutuhan }}</span>
                                                    <input type="hidden"
                                                        name="rencana[{{ $items[0]->warung->id }}][{{ $rencanaId }}][rencana_id]"
                                                        value="{{ $rencanaId }}">
                                                </div>
                                                <div class="flex items-center space-x-2">
                                                    <span class="text-xs text-gray-500 w-16">Jml. Kirim:</span>
                                                    <input type="number"
                                                        name="rencana[{{ $items[0]->warung->id }}][{{ $rencanaId }}][jumlah_kirim]"
                                                        value="{{ $jumlahKebutuhan }}" min="0"
                                                        max="{{ $jumlahKebutuhan }}" disabled required
                                                    ...
                                                    <input type="hidden"
                                                        name="rencana[{{ $items[0]->warung->id }}][{{ $rencanaId }}][id_barang]"
                                                        value="{{ $barangId }}">
                                                    ...
                                                    <select
                                                        ...
                                                        <option value="" disabled selected>Pilih Sumber Stok...
                                                        </option>
                                                    </select>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            allTransactions.forEach(trx => {
                ...
                const barangId = trx.id_barang; // pakai ini, bukan trx.id_barang
            ...
            const warungOptions = Object.entries(warungs).map(([id, nm]) => `<option value="${id}">${nm}</option>`)
                .join('');
            ...
            function recalculateGlobalStock() {
                ...
                document.querySelectorAll('.chk-rencana-warung:checked').forEach(chk => {
                    ...
                    warungBlock.querySelectorAll('.rencana-item').forEach(itemEl => {
                        const trxSelect = itemEl.querySelector(
                            '.rencana-trx-select:not(:disabled)');
                        ...
                        if (trxSelect && qtyInput) {
                            ...
                            if (trxId && qty > 0) {
                                currentStockSisa[trxId] = Math.max(0, currentStockSisa[trxId] -
                                    qty);
                            }
                        }
                    });
                ...
                document.querySelectorAll('.chk-rencana-warung:checked').forEach(chk => updateRencanaWarung(chk
                    .dataset.warungId, false));
    ...
    <div class="flex space-x-2 items-center delivery-row" data-trx="${trxId}" data-row-id="${rowId}">
        <select name="transaksi[${trxId}][details][${rowId}][id_warung]" required
        ...
        <input type="hidden" name="transaksi[${trxId}][id_barang]" value="${stockByBarang[Object.keys(stockByBarang).find(bid => stockByBarang[bid].some(s => s.id == trxId))]?.[0].id_barang ?? ''}">
...
`;
            ...
            function updateWarungOptions(trxId) {
                ...
                allSelects.forEach(select => {
                    if (select.value) selectedWarungs.add(select.value);
                });
            ...
            function updateRencanaOptionsGlobal() {
                ...
                allRencanaSelects.forEach(selectEl => {
                    ...
                    [...selectEl.options].forEach(option => {
                        ...
                        option.textContent =
                            `Stok #${trxId} (${sisa} pcs) @Rp${numberWithCommas(stock.harga)}`;
                    ...
                    if (currentValue && currentStockSisa[currentValue] <= 0 && selectEl.disabled ===
                        false) {
                        selectEl.classList.add('border-red-500');
                    }
                });
                ...
                items.forEach(itemEl => {
                    ...
                    if (isChecked) {
                        ...
                        if (trxId) {
                            ...
                            const stokAwal = Object.values(stockByBarang).flat().find(s => s.id == trxId)
                                .jumlah_awal;
                            ...
                            document.querySelectorAll('.chk-rencana-warung:checked').forEach(otherChk => {
                                ...
                                if (otherWarungId !== warungId.toString()) {
                                    otherChk.closest('.rencana-warung-block').querySelectorAll(
                                        '.rencana-item').forEach(otherItem => {
                                        if (otherItem.querySelector('.rencana-trx-select')
                                            .value == trxId) {
                                            alokasiRencanaLain += +otherItem.querySelector(
                                                '.rencana-qty-input').value;
                                        }
                                    });
                                }
                            });
                            ...
                            document.querySelectorAll(`.chk-trx:checked[data-id="${trxId}"]`).forEach(
                                chkKiri => {
                                    const containerKiri = document.getElementById(
                                        `deliveries-${trxId}`);
                                    containerKiri.querySelectorAll('.qty-input:not(:disabled)').forEach(
                                        qtyInputKiri => {
            ...
            function updateSubmitRencana() {
                ...
                const canSubmit = checkedWarungs.length > 0 && checkedWarungs.every(c => c.dataset.valid ===
                    "true");
            ...
            document.addEventListener('click', e => {
                if (e.target.classList.contains('btn-add')) {
                    ...
                    const max = +document.getElementById(`sisa-${id}`).dataset.max;
                    document.getElementById(`deliveries-${id}`).insertAdjacentHTML('beforeend',
                        createDeliveryRow(id, max));
                    updateSisa(id);
                }
            ...
            formKirim.addEventListener('submit', e => {
                ...
                if (btnSubmitMassal.disabled) {
                    e.preventDefault();
                    alert(
                        'Terdapat data pengiriman yang tidak valid atau kuantitas melebihi stok yang tersedia.'
                        );
                }
            });
            ...
            formRencana.addEventListener('submit', e => {
                if (btnSubmitRencana.disabled) {
                    e.preventDefault();
                    alert(
                        'Terdapat rencana belanja yang tidak valid. Pastikan semua item memiliki sumber stok dan jumlah kirim yang sesuai dengan sisa stok global.'
                        );
                }
            });
