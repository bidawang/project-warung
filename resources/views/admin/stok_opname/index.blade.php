        ...
        <main class="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div class="bg-white shadow-xl border border-gray-200 rounded-lg p-8 max-w-lg mx-auto mt-10 text-center">
                    <svg class="mx-auto h-12 w-12 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 001 1h3m-6-10v10a1 1 0 01-1 1h-3">
                        </path>
                    </svg>
                    <h2 class="text-2xl font-bold mt-4 mb-2 text-gray-800">Pilih Warung untuk Stok Opname</h2>
                    <p class="text-gray-600 mb-6">Silakan pilih warung yang ingin Anda periksa datanya atau masukkan opname
                        baru.</p>

                    <div class="grid gap-4">
                        @foreach ($listWarung as $warung)
                            <a href="{{ route('admin.stokopname.index', ['id_warung' => $warung->id]) }}"
            ...
            @if ($activeWarungId)
                ...
                <div class="flex border-b border-gray-300">
                    @php
                        // Ambil parameter id_warung untuk semua link tab
                        $warungParam = ['id_warung' => $activeWarungId];
                        ...
                        if (Auth::user()->role === 'admin' && $listWarung->count() > 1) {
                            ...
                            foreach ($listWarung as $warung) {
                                ...
                                $link = route(
                                    'admin.stokopname.index',
                                    array_merge(request()->except(['id_warung', 'tab', 'tanggal']), [
                                        'id_warung' => $warung->id,
                                    ]),
                                );
                                echo '<a href="' .
                   ...
                   transition duration-150 ease-in-out">
                    ...
                    @if ($tanggalFilter)
                        <a href="{{ route('admin.stokopname.index', array_merge($warungParam, ['tab' => 'riwayat'])) }}"
                            class="ml-auto py-2 text-sm font-semibold text-blue-600 hover:text-blue-800">
                    ...
                    <section class="bg-white shadow-xl border border-gray-200 rounded-lg p-6">
                        ...
                        @if (!$canInputToday && $lastOpnameDate)
                            <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-4" role="alert">
                                ...
                                <p>Opname terakhir untuk warung ini dilakukan pada tanggal
                                    **{{ $lastOpnameDate->translatedFormat('d F Y') }}**.
                                    Saat ini baru **{{ $daysSinceLastOpname }} hari** berlalu. Anda dapat melakukan Opname
                                    berikutnya minimal **2 hari** setelah opname terakhir.</p>
                            </div>
                        @elseif ($canInputToday && $lastOpnameDate)
                            <div class="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4 mb-4" role="alert">
                                <p class="font-bold">Info Opname</p>
                                <p>Opname terakhir dilakukan pada tanggal
                                    **{{ $lastOpnameDate->translatedFormat('d F Y') }}**. Anda **dapat** melakukan input
                                    opname hari ini.</p>
                            </div>
                        @elseif ($canInputToday && !$lastOpnameDate)
                            <div class="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4 mb-4" role="alert">
                                <p class="font-bold">Info</p>
                                <p>Saat ini belum ada riwayat stok opname untuk warung ini. Input opname pertama Anda
                                    sekarang!</p>
                            </div>
                        @endif
                                    ...
                                    <tbody>
                                        @forelse ($stokSekarang as $stok)
                                            <tr class="border-b hover:bg-gray-50">
                                                ...
                                                <td class="py-3 px-6 text-center font-mono text-gray-800">
                                                    {{ $stok['stok_sistem'] }}</td>
                                                <td class="py-3 px-6 text-center">
                                                    <input type="number" name="jumlah[{{ $stok['id_stok_warung'] }}]"
                                                        min="0"
                                                        ...
                                                        value="{{ old('jumlah.' . $stok['id_stok_warung'], $stok['jumlah_opname']) }}"
                                                        {{-- Ditambahkan $stok['jumlah_opname'] untuk pre-fill nilai jika sudah diopname hari ini --}} {{ $canInputToday ? '' : 'disabled' }}>
                                                    <input type="hidden" name="id_stok_warung[]"
                                                        value="{{ $stok['id_stok_warung'] }}">
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="3" class="text-center py-4 text-gray-500">Tidak ada data
                                                    stok warung yang perlu diopname.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                ...
                @if ($activeTab === 'riwayat' || $tanggalFilter)
                    <section class="bg-white shadow-xl border border-gray-200 rounded-lg p-6">
                        <h2 class="text-xl font-semibold mb-4 border-b pb-2">
                            Riwayat Stok Opname
                            {{ $tanggalFilter ? 'pada Tanggal ' . \Carbon\Carbon::parse($tanggalFilter)->translatedFormat('d F Y') : '(Minus Tertinggi)' }}
                        </h2>
                        ...
                        <div class="flex flex-wrap gap-2 mb-6">
                            ...
                            @empty
                                <p class="text-sm text-gray-500">Belum ada riwayat opname yang tersimpan untuk warung ini.
                                </p>
                            @endforelse
                        </div>
                            ...
                            <table class="min-w-full text-sm text-gray-600 border rounded-lg overflow-hidden">
                                ...
                                <tbody>
                                    @forelse ($stokSekarang as $stok)
                                        ...
                                        <tr class="border-b hover:bg-gray-50">
                                            <td class="py-3 px-6 font-medium">{{ $stok['nama_barang'] }}</td>
                                            <td class="py-3 px-6 text-center font-mono">{{ $stokSistem }}</td>
                                            {{-- Menampilkan Stok Sistem (jumlah_sebelum) --}}
                                            ...
                                            <td class="py-3 px-6 text-center">
                                                <span
                                                    class="px-3 py-1 text-xs rounded-full font-semibold {{ $statusColor }}">
                                                    {{ $statusText }}
                                                </span>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="5" class="text-center py-4 text-gray-500">Tidak ada data stok
                                                warung saat ini.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
