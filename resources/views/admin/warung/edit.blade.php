    ...
    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6 md:p-10">
        <div class="container mx-auto">
            <div class="bg-white shadow-md rounded-lg p-6 md:p-8">
                <h2 class="text-2xl md:text-3xl font-bold text-center text-gray-800 mb-6">Edit Warung</h2>
                ...
                <form action="{{ route('admin.warung.update', $warung->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select class="block w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500" id="id_user" name="id_user" required>
                            ...
                            <option value="{{ $user->id }}" {{ $warung->id_user == $user->id || old('id_user') == $user->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <select class="block w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500" id="id_area" name="id_area" required>
                            ...
                            <option value="{{ $area->id }}" {{ $warung->id_area == $area->id || old('id_area') == $area->id ? 'selected' : '' }}>
                                {{ $area->area }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500" id="nama_warung" name="nama_warung" value="{{ old('nama_warung', $warung->nama_warung) }}" required>
                        @error('nama_warung')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500" id="modal" name="modal" value="{{ old('modal', $warung->modal) }}" required>
                        @error('modal')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500" id="keterangan" name="keterangan">{{ old('keterangan', $warung->keterangan) }}</textarea>
                        @error('keterangan')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
