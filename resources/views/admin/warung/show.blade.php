                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-6">
                        ...
                        <div class="border-t border-gray-200 pt-4 mb-4">
                            <h5 class="text-lg font-medium text-gray-700">Modal Awal</h5>
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 italic">
                                {{ $warung->keterangan ?? 'Tidak ada keterangan.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    {{-- KANAN: LABA --}}
                    <div class="bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-xl shadow-xl p-6 text-white">
                        <h3 class="text-xl font-bold mb-6">Ringkasan Laba</h3>

                        <div class="space-y-4">

                            <div class="flex justify-between">
                                <span class="opacity-80">Total Penjualan</span>
                            ...
                            <div class="flex justify-between">
                                <span class="opacity-80">Total Modal</span>
                                <span class="font-bold text-red-300">
                                    Rp {{ number_format($totalModal, 0, ',', '.') }}
                                </span>
                            </div>

                            <div class="border-t border-indigo-400 pt-4 flex justify-between items-center">
                                <div>
                                    <p class="text-sm opacity-80">Laba Bersih</p>
                                    <p class="text-2xl font-black">
                                        Rp {{ number_format($labaBersih, 0, ',', '.') }}
                                    </p>
                                </div>

                                <div class="bg-white/20 px-3 py-1 rounded-lg text-sm font-bold">
                                    @if ($labaKotor > 0)
                                        {{ number_format(($labaBersih / $labaKotor) * 100, 1) }}%
                                    @else
                                        0%
                                    @endif
                                    Margin
                                            ...
                                            <tbody class="bg-white divide-y divide-gray-200">
                                                @forelse ($tabData['list'] as $barang)
                                                    <tr class="hover:bg-gray-50">
                                                        <td class="px-6 py-4 text-sm font-medium">{{ $loop->iteration }}
                                                        </td>
                                                        <td class="px-6 py-4 text-sm">{{ $barang->nama_barang ?? '-' }}
                                                        </td>
                                                        <td
                                                            class="px-6 py-4 text-sm {{ $status === 'kosong' || ($status === 'semua' && ($barang->stok_saat_ini ?? 0) <= 0) ? 'font-bold text-red-500' : '' }}">
                                                            {{ $barang->stok_saat_ini ?? 0 }}
                                                        </td>
                                                        <td class="px-6 py-4 text-sm">Rp
                                                            {{ number_format($barang->harga_sebelum_markup ?? 0, 0, ',', '.') }}
                                                        </td>
                                                        ...
                                                        <td class="px-6 py-4 text-sm font-semibold">
                                                            {{-- Persentase Laba --}}
                                                            <span
                                                                class="{{ str_contains($barang->persentase_laba, '-') ? 'text-red-600' : 'text-blue-600' }}">
                                                            ...
                                                            @if (($barang->harga_satuan ?? 0) > 0)
                                                                ...
                                                                <div
                                                                    class="mt-1 text-xs font-normal text-gray-500 leading-tight">
                                                                    <span
                                                                        class="{{ $labaAwal < 0 ? 'text-red-600' : 'text-green-600' }} font-bold">
                                                                        Rp {{ number_format($labaAwal, 0, ',', '.') }}
                                                                    </span>
                                                                    @if ($labaAkhir !== $labaAwal)
                                                                        -
                                                                        <span
                                                                            class="{{ $labaAkhir < 0 ? 'text-red-600' : 'text-green-600' }} font-bold">
                                                        ...
                                                        <td class="px-6 py-4 text-sm">
                                                            <ul class="list-none space-y-2 mb-2">
                                                                @forelse($barang->kuantitas as $kuantitas)
                                                                    <li
                                                                        class="flex justify-between items-center text-xs p-1 rounded-md bg-gray-50 hover:bg-gray-100 transition">
                                                                        {{-- Informasi Kuantitas --}}
                                                                        <span class="truncate pr-2">
                                                                            {{ $kuantitas->jumlah }} unit:
                                                                            <span class="font-semibold text-gray-700">Rp
                                                                                {{ number_format($kuantitas->harga_jual, 0, ',', '.') }}</span>
                                                                        </span>
                                                                    </li>
                                                                @empty
                                                            ...
                                                            @if ($barang->stok_saat_ini > 0)
                                                                <form
                                                                    action="{{ route('admin.kuantitas.create', ['id_stok_warung' => $barang->id_stok_warung]) }}"
                                                                    method="GET" class="mt-2">
                                                                    @csrf
                                                                    <input type="hidden" name="id_stok_warung"
                                                                        value="{{ $barang->id_stok_warung }}">
                                                                    <button type="submit"
