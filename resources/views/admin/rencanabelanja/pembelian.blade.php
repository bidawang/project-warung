...
@section('content')
    ...
    <div class="p-4 bg-gray-50 min-h-screen" x-data="pembelianForm()">
                    ...
                    @foreach ($totalKebutuhan as $g => $item)
                        <div class="bg-white border rounded-lg overflow-hidden shadow-sm transition-all"
                                ...
                                <div class="flex items-center space-x-4">
                                    ...
                                    <label
                                        ...
                                        <input type="checkbox" x-model="isSkipped" name="items[{{ $g }}][skip]"
                                            value="1"
                                            class="mr-2 w-4 h-4 rounded border-none text-red-600 focus:ring-0">
                                        SKIP ITEM
                                    </label>
                                </div>
                            ...
                            <div x-show="!isSkipped" x-collapse>
                                    ...
                                    <tbody class="divide-y divide-gray-100">
                                        <template x-for="(row, index) in purchases" :key="index">
                                            <tr class="hover:bg-blue-50/50 transition">
                                                ...
                                                <td class="p-2">
                                                    <input type="number"
                                                        :name="`items[${id}][purchases][${index}][harga]`"
                                ...
                                <div
                                    class="bg-gray-50 p-2 flex justify-between items-center text-[11px] font-bold border-t">
                                    <div class="flex space-x-4">
                                        ...
                                        <span class="text-blue-600 uppercase">Total Qty: <span
                                                x-text="totalQty"></span></span>
                                    </div>
                                    <div class="flex items-center space-x-3">
        ...
        @else
            <div
                class="bg-white rounded-xl shadow-sm p-12 flex flex-col items-center justify-center text-center border-2 border-dashed border-gray-200">
                <div class="bg-blue-50 p-6 rounded-full mb-4">
                    <i class="fas fa-clipboard-check text-5xl text-blue-400"></i>
                </div>
                <h2 class="text-2xl font-bold text-gray-800">Semua Rencana Terpenuhi</h2>
                <p class="text-gray-500 mt-2 max-w-sm">
                    Tidak ada daftar belanja yang perlu diproses saat ini. Semua kebutuhan barang sudah tercatat atau belum
                    ada rencana baru.
                </p>
                {{-- <div class="mt-6 flex gap-3">
                    <a href="{{ route('admin.rencana.index') }}"
                        class="bg-gray-800 text-white px-6 py-2 rounded-lg font-medium hover:bg-black transition">
                        Lihat Riwayat
                    </a>
                    <a href="{{ url()->previous() }}"
                ...
                addRow() {
                    ...
                    this.purchases.push({
                        area_id: this.areas.length === 1 ?
                            this.areas[0].id : this.areas[0]?.id ?? '', // ← AUTO ISI
                        qty: suggestedQty,
