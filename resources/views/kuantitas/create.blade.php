...
<div class="container">
    ...
    <form action="{{ route('kuantitas.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="number" name="harga_jual" id="harga_jual" class="form-control" min="{{ $hargaSatuan }}" required>
            <small class="form-text text-muted">Harga satuan per unit: Rp {{ number_format($hargaSatuan, 0, ',', '.') }}</small>
            @else
